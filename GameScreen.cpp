...
GameScreen:: GameScreen(const uint16_t windowWidth, const uint16_t windowHeight, const uint8_t gridSpacing) 
             : ScreenInterface(windowWidth, windowHeight, "GAME")
{
...
CustomReturnCode GameScreen::run(SDL_Renderer* renderer, const int mouseX, const int mouseY)
{
   // need to come up with better solution than this
   return run(renderer); 
}